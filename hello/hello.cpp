...
void integerOperations(u64 partyIdx)
{
	...
	i64 prodVal;
	enc.revealAll(runtime, sharedInt, prodVal).get();
	cout << prodVal << std::endl;;
...
//  and then perform them.<< std::endl;
//
...
int main(int argc, char* argv[]){
	long party_id = strtol(argv[1], NULL,10) ;
	cout << party_id << std::endl;
	integerOperations(   party_id ) ;
	return 0;
}